...
/**
 * Provides a form for reverting a Billing revision.
 *
 * @ingroup billing
 */
class BillingEntityRevisionRevertForm extends ConfirmFormBase {
  ...
  public function getFormId() {
    return 'billing_entity_revision_revert_confirm';
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->logger('content')->notice('Billing: reverted %title revision %revision.', ['%title' => $this->revision->label(), '%revision' => $this->revision->getRevisionId()]);
    $this->messenger()->addMessage(t('Billing %title has been reverted to the revision from %revision-date.', ['%title' => $this->revision->label(), '%revision-date' => $this->dateFormatter->format($original_revision_timestamp)]));
   ...
   *
   * @param \Drupal\billing\Entity\BillingEntityInterface $revision
   ...
   */
  protected function prepareRevertedRevision(BillingEntityInterface $revision, FormStateInterface $form_state) {
